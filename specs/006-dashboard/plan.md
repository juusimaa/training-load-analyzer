...
**Five items were put to the developer rather than settled here**, under Principles VII and III, and
all five were answered on 2026-09-18. Four amended the specification before any task was generated; the
fifth (R23) was found afterwards by `/speckit-analyze`, and is recorded alongside them rather than
quietly fixed. They are in
[Amendments made during planning and analysis](#amendments-made-during-planning-and-analysis).

Full rationale, alternatives and revisit triggers for all twenty-three decisions are in
...
**Scale/Scope**: 2 new projects, 3 new package references, ~12 new types and 6 components, 20
functional requirements (13 original plus 7 added during planning), 7 success criteria, 5 user stories.
...
| **VII. Specification Adherence** | This gate did the most work in this feature, as it did in 005. Designing against the requirements found **a direct contradiction with a shipped feature** (FR-005's 5% against feature 004's 15%-plus-50-points), **a value the whole system needs and nobody owns** (the maximum heart rate, declined three times by design), **an unrunnable requirement** (reconnect, with nowhere to reconnect from), and **an unmeetable success criterion** (SC-005 against Strava's documented rate limits). None was settled in code. All four were written up with options and a recommendation, put to the developer, answered, and written into the specification before tasks were generated. **It also missed one.** FR-012 says state persists in browser session or storage; the design used a server-side singleton and no artifact said so, which is the silent resolution this principle exists to prevent. It was caught by `/speckit-analyze` and closed the same way as the other four (R23) — but it was caught *after* tasks were generated, not during the gate, and the gate is recorded here as having let it through. | ⚠️ **PASS with one item missed and since closed** |
...
**Two gates carry a caveat rather than a clean pass.** Principle I's risk is that markup work invites
...
Principle VII's caveat is a miss rather than a risk: four specification problems were found and
escalated during planning, and a fifth — FR-012's browser storage — was resolved silently in the design
and only caught afterwards by `/speckit-analyze`. It is closed (R23), and the gate is left marked
because the useful record is not that the problem was fixed but that this gate did not catch it.
...
## Amendments made during planning and analysis

Five items were raised with the developer and answered on 2026-09-18. All five changed the
specification. The first four were raised during planning; **R23 was not** — it was found by
`/speckit-analyze` afterwards, and is listed here rather than quietly fixed, because an amendments
table that records four while a fifth was settled in silence is a worse record than one that records
five. Each is argued in full in
[research.md](./research.md).
...
| **R23** | FR-012 required state to persist "on athlete's device (browser session or storage)". Under Interactive Server the components run on the server and a reload is a **new circuit**, so browser storage would mean a JavaScript interop boundary written solely to re-tell the server something it already knows. The design used a server-side singleton and no artifact said so — the one item of the five settled quietly rather than raised, and it was caught by `/speckit-analyze` rather than during planning. | **Server-held state.** The `SyncCoordinator` singleton outlives any circuit and is what a reloaded page reads; figures are recomputed per load, which R16 measured as affordable. No browser storage, and therefore no JavaScript interop — which also keeps R8's "no JavaScript" and SC-007 intact. | **FR-012** rewritten, new **FR-012a**, two Assumptions |
...
└── TrainingLoadAnalyzer.Web/                    # NEW - the host (project plan 22.1, research R1)
    ...
    │   │   ├── Display.cs                       # NEW - fixed-culture rounding (C99, C100, research R9)
...
└── TrainingLoadAnalyzer.Web.Tests/              # NEW - Microsoft.NET.Sdk.Razor (research R10)
    ├── Fakes/
    │   ├── FixedLocalClock.cs                   # NEW - pins the instant AND the zone (research R22)
    │   ├── WebAppFactory.cs                     # NEW - WebApplicationFactory<Program> (research R18)
    │   ├── SqliteFixture.cs                     # LINKED from 005's test project, not copied
    │   └── StubHttpMessageHandler.cs            # LINKED from 005's test project, not copied
    ├── StartupTests.cs                          # NEW - FR-015, C73, C77, C86 (research R12, R20)
    ...
    ├── DisplayFormatTests.cs                    # NEW - SC-002 under a comma-decimal culture (C99)
    ├── WeeklyLoadAndTrendTests.cs               # NEW - US2: FR-004, FR-005, FR-005a (C82)
    ├── RecentActivitiesTests.cs                 # NEW - US4: FR-007, FR-008 (C84)
